...
function StatusRow({ label, status }) {
    ...
    return (
        <div className="flex items-center justify-between gap-3 border-b border-outline-variant/30 py-2.5">
            ...
            <span className={`inline-flex items-center gap-1.5 text-[11px] font-extrabold uppercase tracking-wide px-2.5 py-1 rounded-full border ${pillClass}`}>
...
function toTechCheckStatus(tech) {
    return [
        { label: 'https',         status: tech?.ssl?.https === true },
        { label: 'favicon',       status: tech?.favicon?.exists === true },
        { label: 'sitemap',       status: tech?.sitemap?.exists === true },
        { label: 'hreflang',      status: tech?.hreflang?.exists === true },
        { label: 'canonical',     status: tech?.canonical?.exists === true },
        { label: 'open graph',    status: tech?.open_graph?.exists === true },
        { label: 'robots txt',    status: tech?.robots_txt?.exists === true && tech?.robots_txt?.blocks_all_crawlers !== true },
        { label: 'meta robots',   status: tech?.meta_robots?.is_noindex !== true },
        { label: 'twitter card',  status: tech?.twitter_card?.exists === true },
        { label: 'schema markup', status: tech?.schema_markup?.exists === true },
        { label: 'viewport meta', status: tech?.viewport_meta?.exists === true },
    ];
}
...
export default function Audit() {
    ...
    // ── Single unified PDF download used by both the button and auto-download ──
    // Double rAF ensures React has flushed the DOM and the browser has painted
    // before html2canvas tries to capture the #audit-report-container element.
    const triggerPdfDownload = useCallback((resultData, rawUrl) => {
        return new Promise((resolve) => {
            setPdfLoading(true);
            requestAnimationFrame(() => {
                requestAnimationFrame(async () => {
                    try {
                        console.log('Triggering PDF download for:', rawUrl);
                        const success = await downloadPDF(
                            'audit-report-container',
                            `SEO_Report_${getHostname(resultData?.url || rawUrl)}.pdf`
                        );
                        if (!success) {
                            setError('PDF generation failed. Please check the browser console for details.');
                        }
                    } catch (err) {
    ...
    // Auto-analyze when a URL is passed via navigation state
    useEffect(() => {
    ...
    // Auto-download PDF once the result is ready and the flag is set
    useEffect(() => {
        if (result && location.state?.downloadPdfImmediately && !autoDownloadDone) {
            triggerPdfDownload(result, url).then(() => setAutoDownloadDone(true));
        }
    }, [result, location.state?.downloadPdfImmediately, autoDownloadDone, url, triggerPdfDownload]);
    ...
    return (
        <AuditErrorBoundary>
            <div className="px-4 py-6 sm:p-8 lg:p-10 max-w-7xl mx-auto w-full space-y-6 sm:space-y-8">
                ...
                {/* Download button — visible once result is ready */}
                {result && lastAnalyzed && (
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            onClick={() => triggerPdfDownload(result, url)}
                            disabled={pdfLoading}
                ...
                {loading && (
                    <section className="bg-surface-container-lowest border border-outline-variant/40 rounded-[24px] p-6 sm:p-10">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="text-base font-bold text-on-surface">Analyzing your website...</div>
                                <div className="text-sm text-on-surface-variant mt-1">This may take a few seconds.</div>
                            </div>
                        </div>
                    </section>
                )}

                {/* ── Audit report — this element is captured by html2canvas ── */}
                {result && (
                    <section id="audit-report-container" className="space-y-6 sm:space-y-8 bg-background p-4 md:p-6 rounded-[28px]">
                                        ...
                                        <div className="mt-6 pt-4 border-t border-[#eeedf5]">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="text-sm font-bold text-on-surface">
                                                    {result.pagespeed?.mobile_friendly === true ? 'Yes'
                                                        : result.pagespeed?.mobile_friendly === false ? 'No' : 'N/A'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="bg-surface-container-lowest border border-outline-variant/40 rounded-[24px] p-5 sm:p-8 overflow-x-auto">
                                ...
                                <p className="text-xs text-on-surface-variant mb-4">
                                    ...
                                    {result.keyword_search_volume && result.serp_snapshot.primary_query && (
                                        <span className="ml-2">· Est. volume: {result.keyword_search_volume[result.serp_snapshot.primary_query] ?? '—'}</span>
                                    )}
                                </p>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-outline-variant/30">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {result.serp_snapshot.related_searches.slice(0, 10).map((q, j) => (
                                                <span key={j} className="text-xs px-2 py-1 rounded-full bg-[#e8f0fe] text-[#174ea6] font-semibold">{q}</span>
                                            ))}
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <div className="text-[16px] font-extrabold text-on-surface">
                                            {(typeof result.content_suggestions?.title === 'object' ? JSON.stringify(result.content_suggestions.title) : result.content_suggestions?.title) || 'N/A'}
                                        </div>
                                    </div>
                                    <div className="pt-6 border-t border-[#eeedf5]">
                                        ...
                                        <div className="text-[14px] font-semibold text-on-surface-variant leading-relaxed">
                                            {(typeof result.content_suggestions?.meta_description === 'object' ? JSON.stringify(result.content_suggestions.meta_description) : result.content_suggestions?.meta_description) || 'N/A'}
                                        </div>
                                    </div>
                                    <div className="pt-6 border-t border-[#eeedf5]">
                                        <div className="text-sm font-bold text-on-surface-variant mb-2">Blog Intro</div>
                                        <div className="text-[14px] font-semibold text-on-surface-variant leading-relaxed">
                                            {(typeof result.content_suggestions?.blog === 'object' ? JSON.stringify(result.content_suggestions.blog) : result.content_suggestions?.blog) || 'N/A'}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    {result.keyword_opportunities.slice(0, 3).map((k, i) => (
                                        <div key={i} className="bg-[#fcfcff] border border-[#eeedf5] rounded-[18px] p-5">
                                            ...
                                            <div className="text-sm font-extrabold text-on-surface mt-2">Position: {k.your_position ?? 'N/A'}</div>
                                            <div className="mt-3">
                                                <div className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1">Suggestion</div>
                                                <div className="text-[13px] font-semibold text-on-surface-variant leading-relaxed">
                                                    {(typeof k.suggestion === 'object' ? JSON.stringify(k.suggestion) : k.suggestion) || 'N/A'}
                                                </div>
                                            </div>
                                            <div className="mt-3">
                                                <div className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1">Action</div>
                                                <div className="text-[13px] font-semibold text-on-surface-variant leading-relaxed">
                                                    {(typeof k.action === 'object' ? JSON.stringify(k.action) : k.action) || 'N/A'}
                                                </div>
                                            </div>
                                        </div>
                                    ))}
